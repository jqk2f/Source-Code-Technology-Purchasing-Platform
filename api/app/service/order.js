...
class OrderService extends BaseService {
  ...
  async createDirect(customerId, payload) {
    const sourceType = payload.sourceType === "service" ? "service" : "product";
    const sourceId = Number(payload.sourceId);
    const quantity = sourceType === "product" ? Math.max(Number(payload.quantity) || 1, 1) : 1;
    const item =
      sourceType === "service"
        ? await this.first("SELECT * FROM services WHERE id=:sourceId AND status='on_sale' AND deleted_at IS NULL", { sourceId })
        : await this.first("SELECT * FROM products WHERE id=:sourceId AND status='on_sale' AND deleted_at IS NULL", { sourceId });
    if (!item) {
      const error = new Error("商品或服务不存在");
      error.status = 404;
      throw error;
    }

    const itemName = sourceType === "service" ? item.name : item.title;
    const unitPrice = Number(sourceType === "service" ? item.startPrice || 0 : item.price || item.startPrice || 0);
    const totalAmount = unitPrice * quantity;
    const status = totalAmount > 0 ? "pending_payment" : "pending_communication";
    const orderNo = makeNo("ORD");
    const contactLines = [];
    if (payload.contactName) contactLines.push(`联系人：${payload.contactName}`);
    if (payload.contactMobile) contactLines.push(`联系电话：${payload.contactMobile}`);
    if (payload.customerRemark) contactLines.push(`备注：${payload.customerRemark}`);
    const customerRemark = contactLines.join("\n");
    ...
    const result = await this.execute(
      `INSERT INTO orders(
        ...
        :orderNo, :customerId, NULL, :sourceType, :title, :totalAmount, 0, 0,
        :totalAmount, 0, 'unpaid', 'not_started', 'none', :status,
        :paymentInstruction, :customerRemark, '', NOW(), NOW(), NOW()
      )`,
      {
        orderNo,
        customerId,
        ...
        totalAmount,
        status,
        paymentInstruction: totalAmount > 0 ? "请按订单金额付款后提交付款凭证，平台确认后开始交付。" : "该项目需先沟通报价，客服确认后更新订单金额。",
        customerRemark
      }
    );
